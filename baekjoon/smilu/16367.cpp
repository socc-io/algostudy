...
string solve() {
  ...
  while (m--) {
    ...
    sat.add_edge(-ids[0], ids[1]);
    sat.add_edge(-ids[0], ids[2]);
    sat.add_edge(-ids[1], ids[0]);
    sat.add_edge(-ids[1], ids[2]);
    sat.add_edge(-ids[2], ids[0]);
    sat.add_edge(-ids[2], ids[1]);
  }
  auto pred = sat.execute();
  if (pred.empty()) return "";
  string ret(n, ' ');
  for (int i = 0; i < n; i++) {
    const vector<char> k = {'R', 'C', 'B'}; 
    ret[i] = k[pred[i]+1];
  }
  return ret;
}
